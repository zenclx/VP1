...
client.on('interactionCreate', async interaction => {
  ...
  if (customId.startsWith('win|')) {
    ...
    if (roundComplete) {
      const uniqueWinners = [...new Set(bracketRound.map(m => m.winner))];
      ...
      if (uniqueWinners.length === 1) {
        ...
        try { const guild = await client.guilds.fetch(match.guildId); await checkAchievements(client, guild, champion, data); } catch {}
